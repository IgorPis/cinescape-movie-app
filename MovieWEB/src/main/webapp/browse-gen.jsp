...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/ionicons/7.1.0/css/ionicons.min.css">
    
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/styles/general.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/styles/style.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/styles/movie-card-component.css" />
    ...
    <c:set var="selectedGenre" value="All" /> <!-- Default to All Movies -->

	<c:forEach items="${allgenres }" var="g">
	    <c:if test="${param.idGenre == g.idGenre}">
	        <c:set var="selectedGenre" value="${g.name}" />
	    </c:if>
	</c:forEach>
	
	<c:if test="${!empty selectedGenre && empty mostPopularTitle}">
		  <title>${selectedGenre} Movies - CineScape</title>
	</c:if>
        
	<c:if test="${!empty mostPopularTitle && selectedGenre == 'All'}">
      ...
      <section class="section-movies" id="movies">
        <div class="movie-heading-box">
	     ...
	     <form class="filter-box" name="" action="/CineScape/home/getAllMovies" method="get">
		    <div>
		        <select class="genre-select" name="idGenre">
		    ...
		    <div>
		        <select class="sort-select" name="sortOrder">
		            <option value="default">Sort By</option>
		            <option value="A-Z" <c:if test="${param.sortOrder == 'A-Z'}">selected</c:if>>A-Z</option>
		            <option value="Z-A" <c:if test="${param.sortOrder == 'Z-A'}">selected</c:if>>Z-A</option>
		        </select>
		    </div>
          ...
          <form name="" action="/CineScape/home/mostPopular" method="get">
            <button class="btn btn--filter btn--popular">
              <p>Most Popular</p>
              <ion-icon class="flame-icon" name="flame"></ion-icon>
            </button>
          </form>
        </div>
        
		<c:if test="${!empty selectedGenre && empty mostPopularTitle}">
		    <h2 class="heading-most-popular">${selectedGenre} Movies</h2>
		</c:if>
        
		<c:if test="${!empty mostPopularTitle && selectedGenre == 'All'}">
		    <h2 class="heading-most-popular">${mostPopularTitle}</h2>
		</c:if>
        ...
        <div class="movies-container" id="movies-container">
        	<c:if test="${!empty allmovies }">
         	...
         	<c:if test="${!empty moviesByGenre }">
        		<c:forEach items="${moviesByGenre }" var="m">
		          <div class="movie-card">
		            <a class="movie-details-link" data-movie-id="${m.idMovie}" href="/CineScape/user/getMovieDetails?idMovie=${m.idMovie}">
		              <img
		                src="${m.posterPath}"
         	...
         	<c:if test="${!empty mostPopular }">
        		<c:forEach items="${mostPopular }" var="m">
		          <div class="movie-card">
		            <a class="movie-details-link" data-movie-id="${m.idMovie}" href="/CineScape/user/getMovieDetails?idMovie=${m.idMovie}">
		              <img
		                src="${m.posterPath}"
