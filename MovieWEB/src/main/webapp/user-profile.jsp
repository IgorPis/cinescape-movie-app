...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/styles/user-profile.css" />
    ...
    <title>${userById.username} Page - CineScape</title>

    <style>
      .image {
        height: 250px;
        width: 250px;
        background-image: url("${userById.imagePath}");
  ...
  <body>
    ...
    <main>
      <div class="btns-box">
        <form id="deleteUserForm" action="/CineScape/admin/deleteUser/${userById.idUser}" method="post">
          <button type="submit" class="del-btn" onclick="showDeleteUserModal()">
      ...
      <c:if test="${not empty deleteAdminMsg}">
		<div class="msg-box">
		   <p class="message">${deleteAdminMsg}</p>
		</div>
		<c:remove var="deleteAdminMsg" scope="session"/>
	  </c:if>
        ...
        <div class="ticket">
          <div class="left">
            ...
            <div class="ticket-info">
              <p class="role-text">
                <span>Role</span>
                <span class="role">${userById.role.role}</span>
                <span>ID:${userById.idUser}</span>
              </p>
              <div class="user-name">
              ...
              <div class="txt-sm">
                <a href="/CineScape/admin/editUserPage/${userById.idUser}" class="link edit-btn"
          ...
          <div class="right">
            ...
            <div class="right-info-container">
              ...
              <div class="txt-sm">
                <p>${userById.role.role}</p>
                <p>${userById.firstName} ${userById.lastName}</p>
              </div>
              <div class="img-sm">
                <img
                  src="${userById.imagePath}"
                  alt="User's profile image"
                />
              </div>
              <p class="user-id">#${userById.idUser}</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <div id="deleteUserModal" class="modal">
	   <div class="modal-content">
	      <span class="close-button" onclick="closeDeleteUserModal()">&times;</span>
	      <div class="modal-text-btn-box">
	         <p class="modal-text">Are you sure you want to delete this user?</p>
	         <div class="modal-buttons">
	            <button class="yes" onclick="proceedToDeleteUser()">Yes</button>
	            <button class="no" onclick="closeDeleteUserModal()">No</button>
	         </div>
	      </div>
	   </div>
	</div>
