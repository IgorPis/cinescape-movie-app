...
<html lang="en">
  <head>
    ...
    <c:set var="selectedLang" value="English" /> <!-- Default to English -->

	<c:forEach items="${langs }" var="l">
	    <c:if test="${param.idLang == l.idLanguage}">
	        <c:set var="selectedLang" value="${l.name}" />
	    </c:if>
	</c:forEach>

  <title>${selectedLang} Movies - CineScape</title>
  </head>
  <body>
   <header class="header">
    ...
    <main>
      <section class="section-movies" id="movies">
        ...
        <h2 class="heading-most-popular">${selectedLang} Movies</h2>

        <div class="movies-container">
           <c:if test="${!empty englishMovies}">
        		<c:forEach items="${englishMovies }" var="m">
		    ...
		    <c:if test="${!empty moviesByLanguage}">
		        <c:forEach items="${moviesByLanguage}" var="m">
		            <div class="movie-card">
		                <a class="movie-details-link" data-movie-id="${m.idMovie}" href="/CineScape/user/getMovieDetails?idMovie=${m.idMovie}">
		                    <img src="${m.posterPath}" class="movie-img" alt="Movie poster image" />
		                </a>
		            </div>
		        </c:forEach>
		    </c:if>
